            ...
            <c:if test="${Hworkss.size()!=0}">
                ...
                <div class="layui-card-body">
                    <table class="layui-table layui-form" id="aa">
                        ...
                        <tbody>
                        <c:forEach items="${Hworkss}" var="item">
                            <tr>
                                <td>${item.hno}</td>
                                <td>${item.sno}</td>
                                <td>${item.cno}</td>
                    ...
                    <ul class="pagination">
                        <c:choose>
                            <c:when test="${pageInfo.hasPreviousPage}">
                                <li><a href="/user/selectSGrade?pageNum=1">首页</a></li>
                                <li><a href="/user/selectSGrade?pageNum=${pageInfo.prePage}">上一页</a></li>
                            </c:when>
                            <c:otherwise>
                        ...
                        <c:forEach var = "index" begin="1" end="${pageInfo.pages}">
                            <li class="active"
                                ...
                                <a href="javascript:/user/selectSGrade?pageNum=${index}">${index}</a>
                            </li>
                        </c:forEach>
                        <c:choose>
                            <c:when test="${pageInfo.hasNextPage}">
                                <li><a href="/user/selectSGrade?pageNum=${pageInfo.nextPage}">下一页</a></li>
                                <li><a href="/user/selectSGrade?pageNum=${pageInfo.pages}">尾页</a></li>
                            </c:when>
                            <c:otherwise>
